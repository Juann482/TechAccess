...
public interface AccesoRepository extends JpaRepository<Acceso, Integer> {
	...
	@Query(value = """
			...
			AND (:fechaEgreso IS NULL OR DATE(a.horaEgreso) = :fechaEgreso)
			""")
	Page<Acceso> filtrarUsuariosIngresos(@Param("usuarioId") Integer usuarioId, @Param("nombre") String nombre,
			@Param("documento") String documento, @Param("rol") String rol, @Param("fichaTexto") String fichaTexto, // Cambiado
																													// a
																													// String
																													// para
																													// LIKE
			@Param("fechaIngreso") LocalDate fechaIngreso, @Param("fechaEgreso") LocalDate fechaEgreso,
			Pageable pageable);
	...
	@Query(value = """
			...
			AND (:fechaEgreso IS NULL OR DATE(a.horaEgreso) = :fechaEgreso)
			""")
	Page<Acceso> filtrarUsuariosIngresosBasico(@Param("nombre") String nombre, @Param("documento") String documento,
			@Param("rol") String rol, @Param("fechaIngreso") LocalDate fechaIngreso,
			@Param("fechaEgreso") LocalDate fechaEgreso, Pageable pageable);
	...
	// Filtro por MES
	@Query("SELECT a FROM Acceso a WHERE a.usuario.id = :userId AND MONTH(a.horaIngreso) = :mes AND YEAR(a.horaIngreso) = :anio ORDER BY a.horaIngreso DESC")
	List<Acceso> findByUsuarioAndMes(@Param("userId") Long userId, @Param("mes") int mes, @Param("anio") int anio);

	
	
	// Filtro por RANGO
	@Query("SELECT a FROM Acceso a WHERE a.usuario.id = :userId AND a.horaIngreso BETWEEN :inicio AND :fin ORDER BY a.horaIngreso DESC")
	List<Acceso> findByUsuarioAndRango(@Param("userId") Long userId, @Param("inicio") LocalDateTime inicio,
